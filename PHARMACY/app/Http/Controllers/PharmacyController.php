...
use App\Http\PharmacyController\Storage;
...
class PharmacyController extends Controller
{
  ...
  public function receiveOrder(Request $request)
  {
    ...
    \Log::info('Incoming Order Request:', $request->all());
      ...
      $request->validate([
          ...
          'order_details.*.itemdetails_ID' => 'required|',//exists:item_details,id',
          'order_details.*.quantities' => 'required|numeric',
          'order_details.*.unitprice' => 'sometimes|numeric',
      ...
      $order->paymentstatus = 'pending'; // Payment status is always pending by default
      ...
      foreach ($request->input('order_details') as $item) {
        ...
        $quantities = $item['quantities'];
          ...
          $orderDetail->itemdetails_ID = $item['itemdetails_ID'];
          $orderDetail->quantities = $item['quantities'];
        ...
        if ($order->order_type === 'lmd') {
            DB::table('stocks')
                ->where('itemdetails_ID', $itemdetails_ID)
                ->decrement('stock_qty', $quantities);
  ...
  public function getPharmacyRatingAndReviews()
{
    // Calculate total reviews and average rating for all items in Pharmacy
    $PharmacyRatings = DB::table('itemrating')
        ->join('itemdetails', 'itemrating.itemdetails_ID', '=', 'itemdetails.id') // Join item details
    ...
    return response()->json([
        'total_reviews' => $PharmacyRatings->total_reviews, // Total number of reviews
        'average_rating' => round($PharmacyRatings->average_rating, 2) // Rounded average rating (out of 5)
    ]);
}
...
public function addItemRating(Request $request): JsonResponse
{
    ...
    $existingRating = DB::table('itemrating')
        ...
        ->where('itemdetails_ID', $request->itemdetails_ID)
        ->first();
    ...
    if ($existingRating) {
        return response()->json([
            ...
            'error' => [
                ...
                'details' => [
                    'itemdetails_ID' => $request->itemdetails_ID,
                    'order_id' => $request->id,
                ]
            ]
        ], 409);
  ...
  public function getOrderStatus()
  {
      try {
          ...
          $orders = Order::where('order_type', 'lmd')->get(['id', 'status', 'paymentstatus']);
...
public function getItemsWithDetails()
{
    ...
    $items = DB::table('items')
        ...
        ->select(
            ...
            'itemdetails.id as itemdetails_ID',
    ...
    $formattedItems = $items->groupBy('itemdetails_ID')->map(function ($itemDetails) use ($baseUrl) {
...
///New Ststuses update
 ...
 public function markAsProcessing($id)
 {
    ...
    //  $response = Http::patch("http://192.168.43.63:8000/vendor/order/{$order->suborder_id}/in-progress");
     ...
     return response()->json([
         'success' => true,
         'message' => 'Order marked as processing (Pharmacy) and in-progress (Main)',
...
public function confirmPaymentByCustomer($id)
{
    ...
    if ($order->paymentstatus !== 'pending') {
        return response()->json([
            ...
            'current_status' => $order->paymentstatus
        ], 400);
    }

    $order->paymentstatus = 'confirmed_by_customer';
    $order->save();
...
public function confirmPaymentByDeliveryBoy($id)
{
    ...
    if ($order->paymentstatus !== 'confirmed_by_customer') {
        return response()->json([
            'success' => false,
    ...
    $order->paymentstatus = 'confirmed_by_deliveryboy';
    $order->save();
...
public function confirmPaymentByVendor($id)
{
    ...
    if ($order->paymentstatus !== 'confirmed_by_deliveryboy') {
        return response()->json([
            'success' => false,
    ...
    $order->paymentstatus = 'confirmed_by_vendor';
    $order->save();
...
public function getAllOrdersWithDetailsForPharmacy()
{
    ...
    $orders = DB::table('orders')
        ...
        ->select(
            ...
            'orderdetails.quantities as item_quantity',
    ...
    foreach ($orders as $row) {
        ...
        $grouped[$orderId]['items'][] = [
            'itemdetails_ID' => $row->itemdetails_ID,
...
public function updateSuborderStatusFromPharmacy(Request $request)
{
    ...
    if (!$order) {
        return response()->json([
            'success' => false,
            'message' => 'Order not found in Pharmacy system.',
        ], 404);
    }

    // Step 3: Update status locally in Pharmacy
    if ($statusType === 'payment') {
        ...
        $order->paymentstatus = $incomingStatus;
        $order->save();
    ...
    try {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Pharmacy status updated and sent to LMD.',
            'kfc_order' => [
                'order_id' => $order->id,
                'status' => $order->status,
                'paymentstatus' => $order->paymentstatus,
        ...
        return response()->json([
            'success' => false,
            'message' => 'LMD update failed. Pharmacy status not updated.',
            'error' => $e->getMessage(),
            'lmd_response' => null
        ]);
    }
