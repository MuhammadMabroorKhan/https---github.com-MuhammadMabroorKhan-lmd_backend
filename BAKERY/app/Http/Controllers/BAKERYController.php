...
use App\Http\BAKERYController\Storage;
...
class BAKERYController extends Controller
{
  ...
  public function getBAKERYRatingAndReviews()
{
    // Calculate total reviews and average rating for all items in BAKERY
    $BAKERYRatings = DB::table('itemrating')
    ...
    return response()->json([
        'total_reviews' => $BAKERYRatings->total_reviews, // Total number of reviews
        'average_rating' => round($BAKERYRatings->average_rating, 2) // Rounded average rating (out of 5)
    ]);
}
...
//         ->select('items.id as item_id', 'items.name as item_name', 'items.description', 'itemcategories.name as category_name', 'items.BAKERY_ID')
//         ->get();
...
//  }
 ...
 public function markAsProcessing($id)
 {
    ...
    //  $response = Http::patch("http://192.168.43.63:8000/vendor/order/{$order->suborder_id}/in-progress");
     ...
     return response()->json([
         'success' => true,
         'message' => 'Order marked as processing (BAKERY) and in-progress (Main)',
         'BAKERY_status' => $order->status,
...
public function getAllOrdersWithDetailsForBAKERY()
{
