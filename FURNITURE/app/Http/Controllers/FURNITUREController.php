...
use App\Models\FURNITUREImages;
use App\Models\Item;
...
use App\Http\FURNITUREController\Storage;
...
class FURNITUREController extends Controller
{
  ...
  public function getFURNITURERatingAndReviews()
{
    // Calculate total reviews and average rating for all items in FURNITURE
    $FURNITURERatings = DB::table('itemrating')
    ...
    return response()->json([
        'total_reviews' => $FURNITURERatings->total_reviews, // Total number of reviews
        'average_rating' => round($FURNITURERatings->average_rating, 2) // Rounded average rating (out of 5)
    ]);
}
...
//         ->select('items.id as item_id', 'items.name as item_name', 'items.description', 'itemcategories.name as category_name', 'items.FURNITURE_ID')
//         ->get();
...
//  }
 ...
 public function markAsProcessing($id)
 {
    ...
    //  $response = Http::patch("http://192.168.43.63:8000/vendor/order/{$order->suborder_id}/in-progress");
     ...
     return response()->json([
         'success' => true,
         'message' => 'Order marked as processing (FURNITURE) and in-progress (Main)',
         'FURNITURE_status' => $order->status,
...
public function getAllOrdersWithDetailsForFURNITURE()
{
...
// public function updateSuborderStatusFromFURNITURE(Request $request)
// {
...
//         return response()->json([
//             'success' => false,
//             'message' => 'Order not found in FURNITURE system.',
//         ], 404);
//     }
...
//     return response()->json([
//         'success' => true,
//         'message' => 'FURNITURE status updated and sent to LMD.',
//         'FURNITURE_order' => [
//             'order_id' => $order->id,
//             'status' => $order->status,
//             'payment_status' => $order->payment_status,
...
//     return response()->json([
//         'success' => false,
//         'message' => 'LMD update failed. FURNITURE status not updated.',
//         'error' => $e->getMessage(),
//         'lmd_response' => null
//     ]);
// }
...
public function updateSuborderStatusFromFURNITURE(Request $request)
{
    ...
    // Step 3: Update status locally in FURNITURE
